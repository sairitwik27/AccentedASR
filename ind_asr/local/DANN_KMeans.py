...
class DANN_ASR(nn.Module):
    ...
    def forward(self,xs_pad,ilens,ys_pad,acc_labels):
        ...
        pred_pad, pred_mask = self.E2E.decoder(ys_in_pad, ys_mask, hs_pad, hs_mask)
        ...
        # 5. compute cer/wer
        if self.training or self.E2E.error_calculator is None or self.E2E.decoder is None:
            cer, wer = None, None
        else:
    ...
    def scorers(self):
        """Scorers."""
        return dict(decoder=self.decoder, ctc=CTCPrefixScorer(self.E2E.ctc, self.E2E.eos))
    ...
    def recognize(self, x, recog_args, char_list=None, rnnlm=None, use_jit=False):
        ...
        if self.mtlalpha > 0 and recog_args.ctc_weight == 1.0:
            ...
            hyp = [x for x in filter(lambda x: x != self.E2E.blank, collapsed_indices)]
            nbest_hyps = [{"score": 0.0, "yseq": [self.E2E.sos] + hyp}]
        ...
        if lpz is not None:
            ctc_prefix_score = CTCPrefixScore(lpz.detach().numpy(), 0, self.E2E.eos, numpy)
        ...
        for i in six.moves.range(maxlen):
            ...
            for hyp in hyps:
                ...
                if use_jit:
                    ...
                    local_att_scores = self.E2E.decoder.forward_one_step(
            ...
            if i == maxlen - 1:
                ...
                for hyp in hyps:
                    hyp["yseq"].append(self.E2E.eos)

            # add ended hypothes to a final list, and removed them from current hypothes
            # (this will be a probmlem, number of hyps < beam)
            remained_hyps = []
            for hyp in hyps:
                if hyp["yseq"][-1] == self.E2E.eos:
