...
""" MODIFICATIONS FROM THE ORIGINAL ESPNET CODE,ADDED MY OWN MODULES PLUS TRAINING CHANGES """
...
from load_tr_model import load_trained_model
...
from io_utils_dann_rit import LoadInputsAndTargets
...
def asr_train(args):
    ...
    classifier.load_state_dict(torch.load("/home/ritwikkvs/ritwik/espnet/egs/wsj/ind_asr/classifier_train/snapshot.ep.50")['state_dict'],strict=False)
    classifier.classifier_output = torch.nn.Linear(128,5)
    ...
    #def change_path_IITM(train_json):
    #    x = list(train_json.items())
    #    for i in range(len(x)):         
    #        x[i][1]["input"][0]["feat"] = x[i][1]["input"][0]["feat"].replace("/home/ritwikkvs/ritwik/espnet/egs/wsj/ind_asr","/media/oem/TOSHIB/dump_ind_asr")
    #change_path_IITM(train_json)
        
    with open(args.valid_json, "rb") as f:
        valid_json = json.load(f)["utts"]
    
    train_batch = dict(list(train_json.items())[:64])
    valid_batch = dict(list(valid_json.items())[:64])
    ...
    if args.num_save_attention > 0 and is_attn_plot:
        ...
        if hasattr(model, "module"):
            ...
            att_vis_fn = model.E2E.calculate_all_attentions
            plot_class = model.E2E.attention_plot_class
    ...
    if mtl_mode in ["ctc", "mtl"] and args.num_save_ctc > 0:
        ...
        if hasattr(model, "module"):
            ...
            ctc_vis_fn = model.E2E.calculate_all_ctc_probs
            plot_class = model.E2E.ctc_plot_class
            ...
            if enh_writer is not None:
                ...
                if args.keep_length:
                    if len(org_feats[idx]) < len(enh):
                        ...
                        enh = numpy.pad(enh, padwidth, mode="constant")
