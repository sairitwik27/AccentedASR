...
class Encoder(torch.nn.Module):
    ...
    def __init__(
        ...
        ## My Addition to pick intermediate encoder layers for sharing during DANN

        self.pick_intermediate = False
        if self.pick_intermediate:
            self.int_num_blocks = 6
            self.int_encoders = repeat(
                self.int_num_blocks,
                lambda lnum: EncoderLayer(
                    attention_dim,
                    encoder_selfattn_layer(*encoder_selfattn_layer_args[lnum]),
    ...
    def forward(self, xs, masks):
        ...
        if self.pick_intermediate: ## Sharing the intermediate encoder layers
            int_xs,int_masks = self.int_encoders(xs,masks)
            return xs, masks,int_xs,int_masks
        else:
            return xs, masks
